...
// Clear screen for Linux/Windows
void clear_console() {
#ifdef _WIN32
    system("cls");
#else
...
// Add an exit time for a student
void add_exit_time(struct student *s) {
    ...
    snprintf(log_message, sizeof(log_message), "exit recorded for student: %s", s->name);
    add_log(log_message);
}

// Add an entry time for a student
void add_entry_time(struct student *s) {
    struct timest *time_ptr = s->time_head;
    ...
    char log_message[200];
    snprintf(log_message, sizeof(log_message), "entry recorded for student: %s", s->name);
    add_log(log_message);
}

// Display history of entries and exits for a student
void hist(struct student *s) {
    struct timest *time_ptr = s->time_head;
    ...
    if (time_ptr == NULL) {
        ...
        while (time_ptr != NULL) {
            ...
            // Copy and trim ctime() output for Exit Time
            strncpy(exit_time, ctime(&time_ptr->exit), 29);
            exit_time[24] = '\0'; // Remove newline character
            
            // Copy and trim ctime() output for Entry Time if recorded
            if (time_ptr->entry != 0) {
                strncpy(entry_time, ctime(&time_ptr->entry), 29);
                entry_time[24] = '\0'; // Remove newline character
            } else {
...
// Save data to login.txt
void writeToFile(struct node *head, const char *filename) {
    FILE *file = fopen(filename, "w");
    if (file == NULL) {
        printf(RED "Error: Unable to open file for writing.\n" RESET);
        add_log("Failed to save data to login.txt");
        return;
    }
    ...
    fclose(file);
    add_log("Data saved to login.txt");
}
...
void loadFromFile(struct node **head, const char *filename) {
    ...
    if (file == NULL) {
        ...
        add_log("No previous data found. Started fresh.");
        return;
    }
    ...
    fclose(file);
    add_log("Data loaded from login.txt");
}
...
void reg(struct node **head) {
    ...
    char log_message[200];
    snprintf(log_message, sizeof(log_message), "New user registered: %s (%s)", new_student.name, new_student.roll);
    add_log(log_message);
    writeToFile(*head, "login.txt");
}

// Check if a roll number exists in the list
int is_roll_exists(struct node *head, const char *roll) {
    while (head != NULL) {
        if (strcmp(head->s.roll, roll) == 0) {
            return 1;
...
void delete_user(struct node **head, const char *roll_no) {
    ...
    char log_message[200];
    snprintf(log_message, sizeof(log_message), "User deleted: %s (%s)", temp->s.name, temp->s.roll);
    add_log(log_message);
    writeToFile(*head, "login.txt");
...
// Search for students by a specific date
void search_by_date(struct node *head, int day, int month) {
    struct node *temp = head;
    int found = 0;
...
{   
    ...
    int t; // Threshold time difference in seconds
    scanf("%d", &t);

    struct node *temp = head; // Start at the head of the linked list
    int found = 0; // Flag to check if any student meets the condition
    ...
    while (temp != NULL)
    {
        ...
        while (time_ptr != NULL)
        {
            ...
            // Check if entry time is valid (not equal to a sentinel value like -1 or 0)
            if (time_ptr->entry != (time_t)-1 && time_ptr->entry != 0)
            {
                // Ensure that exit time is greater than entry time
                if (time_ptr->exit > time_ptr->entry)
                {
                    diff = difftime(time_ptr->entry, time_ptr->exit);
                }
                else
                {
                    printf("Warning: Exit time is earlier than entry time for student %s\n", temp->s.name);
                }
            }
            else
            {
                // If entry time is invalid, use current time for calculation
                if (time_ptr->exit > 0) // Check if exit time is valid
                {
                    diff = difftime(time_ptr->exit, time(NULL)); // Use current time if entry is invalid
                }
                else
                {
                    printf("Warning: Invalid entry and exit times for student %s.\n", temp->s.name);
                    diff = 0; // No valid time difference if both are invalid
                }
            }

            // If the time difference exceeds the threshold, print the student's name
            if (diff >= t)
            {
    ...
    // If no student meets the condition
    if (!found)
    {
        printf("No student found with time difference greater than the threshold.\n");
    }
}
...
void fpage(struct node **head) {
    ...
    while (1) {
        if (f_num==0){
            ...
            sleep(1);
            printf(ORANGE"|" RESET "9. " GREEN "Exit" RESET                                ORANGE"                                 |\n" RESET);
            sleep(1);
            printf(BLUE "+========================================+\n" RESET);
            printf(YELLOW "Enter your choice: " RESET                   );
            f_num++;
        ...
        if (scanf("%d", &choice) != 1) {
            // If scanf fails (non-integer input), clear the buffer
            while (getchar() != '\n');  // Clear the buffer
            printf(RED "Invalid input. Please enter a valid choice (1-9).\n" RESET);
            continue; // Prompt the user again
        }
        ...
        switch (choice) {
            ...
            case 3: {
                ...
                while (1) {
                    ...
                    if (month < 1 || month > 12) {
                        clear_console();
                        printf(RED "Invalid month input. Try again.\n" RESET);
                        continue; // Prompt the user again for a valid month
                    }
                ...
                while (1) {
                    ...
                    if (day < 1 || day > 31) {
                        clear_console();
                        printf(RED "Invalid day input. Try again.\n" RESET);
                        continue; // Prompt the user again for a valid day
                    }
            ...
            default:
                clear_console();
                printf(RED "Invalid input. Please enter a valid choice (1-7).\n" RESET);
        }
    }
}
...
int main() {
    ...
    //         fflush(stdout); // Immediately display the output
    //         usleep(50000); // Pause to simulate loading
    //     }
    // }
    clear_console();
